...
<div <?php fl_woocommerce_version_check( '3.4.0' ) ? wc_product_class( $classes, $product ) : post_class( $classes ); ?>>
	...
	<div class="product-small box <?php echo flatsome_product_box_class(); ?>">
		...
		<div class="box-text <?php echo flatsome_product_box_text_class(); ?>">
		    <?php $ngay_cap_nhat=get_field('ngay_cap_nhat');
							    $phien_ban=get_field('phien_ban');?>
							    <?php if($ngay_cap_nhat){?>
							    <div class="tdk-product-loop-custom-product-meta">
							        <span class="last-updated-date"><i class="far fa-calendar-alt"></i><span><?php the_field('ngay_cap_nhat');?></span></span><span class="version"><i class="fas fa-code-branch"></i><?php the_field('phien_ban');?></span></div>
							    <?php }?>
			<?php
